...
{
    uint32 xdata match_temp = 0;
    uint32 xdata period_temp = 0;
    uint16 xdata freq_div = 0;
...
{
    uint32 xdata match_temp;
    uint32 xdata arr = (PWMX_ARRH(pin) << 8) | PWMX_ARRL(pin);
...
void pwm_init(pwm_channel_enum pin, uint32 freq, uint32 duty)
{
    ...
    uint16 xdata match_temp = 0;                                        // 占空比值
    uint32 xdata period_temp = 0;                                       // 周期值
    uint16 xdata freq_div = 0;                                          // 分频值
