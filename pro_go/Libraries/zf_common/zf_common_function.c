...
{
    uint8 xdata sign = 0;                                                       // 标记符号 0-正数 1-负数
    int32 xdata temp = 0;                                                       // 临时计算变量
	zf_assert(str != NULL);
	do
    {
        if(NULL == str)
...
{
    uint8 xdata data_temp[16];                                                  // 缓冲区
    uint8 xdata temp_bit = 0;                                                   // 数字位数
    int32 xdata number_temp = 0;
    zf_assert(str != NULL);
    do
    {
        if(NULL == str)
...
{
    uint8 xdata sign = 0;                                                       // 标记符号 0-正数 1-负数
    float xdata temp = 0.0;                                                     // 临时计算变量 整数部分
    float xdata temp_point = 0.0;                                               // 临时计算变量 小数部分
    float xdata point_bit = 1;                                                  // 小数累计除数
...
void func_float_to_str (char *str, float number, uint8 point_bit)
{
    ...
    uint8 xdata temp_bit = point_bit;                                           // 转换精度位数
...
{
    uint8 xdata sign = 0;                                                       // 标记符号 0-正数 1-负数
    double xdata temp = 0.0;                                                    // 临时计算变量 整数部分
    double xdata temp_point = 0.0;                                              // 临时计算变量 小数部分
    double xdata point_bit = 1;                                                 // 小数累计除数
...
{
    uint32 xdata str_len = strlen(str);                                         // 字符串长
    uint32 xdata result_data = 0;                                               // 结果缓存
    uint8 xdata temp = 0;                                                       // 计算变量
    uint8 xdata flag = 0;                                                       // 标志位
    ...
    while (*format)
    {
        ...
        if ('%' == ret)
        {
            switch (*++ format)
            {
                ...
                case 'c':// 一个字符
                    {
                        int8 xdata ch = (int8)va_arg(arg, uint32);
                        *buff = ch;
                        buff ++;
                        buff_len ++;
                    }
                ...
                case 'i':// 有符号十进制整数
                    {
                        int8 xdata vstr[33];
                        int32 xdata ival = (int32)va_arg(arg, int32);
                        uint8 xdata vlen = number_conversion_ascii((uint32)ival, vstr, 1, 10);
                ...
                {
                    int8 xdata vstr[33];
                    double xdata ival = (double)va_arg(arg, double);
                    uint8 xdata vlen = number_conversion_ascii((uint32)(int32)ival, vstr, 1, 10);
                ...
                case 'u':// 无符号十进制整数
                    {
                        int8 xdata vstr[33];
                        uint32 xdata ival = (uint32)va_arg(arg, uint32);
                        uint8 xdata vlen = number_conversion_ascii(ival, vstr, 0, 10);
                ...
                case 'o':// 无符号八进制整数 
                    {
                        int8 xdata vstr[33];
                        uint32 xdata ival = (uint32)va_arg(arg, uint32);
                        uint8 xdata vlen = number_conversion_ascii(ival, vstr, 0, 8);
                ...
                case 'X':// 无符号十六进制整数
                    {
                        int8 xdata vstr[33];
                        uint32 xdata ival = (uint32)va_arg(arg, uint32);
                        uint8 xdata vlen = number_conversion_ascii(ival, vstr, 0, 16);
