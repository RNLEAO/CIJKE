...
static volatile wireless_uart_auto_baudrate_state_enum xdata wireless_auto_baud_flag = WIRELESS_UART_AUTO_BAUD_RATE_INIT;
static volatile uint8 xdata                             wireless_auto_baud_data[3]  = {0x00, 0x01, 0x03};
#endif
...
uint32 wireless_uart_send_buffer(const uint8 *buff, uint32 len)
{
    ...
    uint16 xdata time_cnt = 0;
	uint16 xdata send_len = 0;
...
//    uart_query_byte(WIRELESS_UART_INDEX, &uart_dat);
    ...
    {
        uint32 xdata wireless_auto_baud_count = 3;
...
uint8 wireless_uart_init (void)
{
    ...
    uint8 xdata rts_init_status = 0;
    uint16 xdata time_count = 0;
